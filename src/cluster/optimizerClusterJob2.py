...
def runClusterJob(allRepertoires, run_id, numOfTrials=100, testCase=False):
    distributionNames = ["componentSizeDistribution"]
    ...
    # if size != len(distributionNames)*len(allRepertoires)*3:
    ...
    distributionName = distributionNames[0]
    ...
    if cluster_rank == 0:
        ...
        if not testCase:  
            with open(f"results_optimizer_{distributionName}_{run_id+(world_rank//3)}.pkl", "wb") as f:
                pickle.dump(study, f)
        else:
            filename = f"proc_{world_rank}_{run_id+(world_rank//3)}.txt"
            print(f"This is testcase. Process rank is {world_rank}. Id is {run_id} and thus filename is {filename}.")

            with open(filename, "w") as f:
                f.write("a")

        print(f"Finished processing for {distributionName} by process {world_rank}")
