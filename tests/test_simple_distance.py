...
class TestSimpleDistance(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.df = df
        self.simple_distance = simple_distance(df_strategy().output())
    ...
    def test_simple_distance_networks(self):
        for dist in makeEnumDict(Matrices):
            df_net = self.simple_distance.createGraph(clonotypes=self.df, distanceFun=sequenceAligner("BLOSUM62").tcr_alig)
