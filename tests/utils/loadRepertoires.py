...
def loadRepertoires():
    ...
    healthy_network_path = test_data_path / "healthy_test_network_1.pkl" #healthy
    ...
    with Session(engine) as session:
        networks = []
        for group, networkPath, networkNum in zip(groups, networkPaths, networkNums):
            name = f"{group} test network_{networkNum}"
